...
const UserProfileScreen = () => {
    ...
    */
    ...
    setTimeout(() => {
      ...
      setFormData({
        ...
        phone: mockProfile.userId.phone || '',
        accountName: mockProfile.userId.accountName || ''
      });
      setLoading(false);
    }, 1000);
  }, [token]);
  ...
  const handleSave = async () => {
    ...
    */
    ...
    // ВРЕМЕННАЯ ЛОГИКА ДЛ�Я ТЕСТИРОВАНИЯ
    setTimeout(() => {
      alert('Профиль обновлён успешно');
      setEditMode(false);
    ...
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">Полное имя</label>
            {editMode ? (
              <input
                type="text"
              ...
              <div className="border border-gray-300 rounded-lg px-3 py-3">
                <span className="text-base text-black">{profile.fullName || 'Не указано'}</span>
              </div>
            )}
          </div>
