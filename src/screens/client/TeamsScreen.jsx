...
const TeamsScreen = () => {
  ...
  const mockTeams = [
    {
      ...
      shortDescription: "Ремонт по хорошим ценам. Быстро, качественно полавершь! Быстро, качественно",
      categories: ["Демонтаж", "Отделка", "Дизайн интерьеров"],
      rating: 5.0,
      reviews: 52,
      location: "Алматы",
    ...
    {
      id: 2,
      account_name: "СтройМастер",
      fullName: "СтройМастер - Профессиональное строительство",
      shortDescription: "Комплексные ремонтные работы любой сложности. Гарантия качества и соблюдение сроков.",
      categories: ["Сантехника", "Электрика", "Отделка"],
      rating: 4.8,
      reviews: 38,
      location: "Алматы",
      previewImage: "/api/placeholder/300/200"
    ...
    {
      id: 3,
      account_name: "ДомМастер",
      fullName: "ДомМастер - Ваш надежный помощник",
      shortDescription: "Качественный ремонт квартир и домов. Индивидуальный подход к каждому клиенту.",
      categories: ["Плитка", "Покраска", "Напольные покрытия"],
      rating: 4.9,
      reviews: 71,
      location: "Алматы",
      previewImage: "/api/placeholder/300/200"
    ...
    {
      id: 4,
      account_name: "РемонтПлюс",
      fullName: "РемонтПлюс - Современные решения",
      shortDescription: "Современный подход к ремонту. Используем качественные материалы и новейшие технологии.",
      categories: ["Дизайн интерьеров", "Отделка", "Электрика"],
      rating: 4.7,
      reviews: 29,
      location: "Алматы",
      previewImage: "/api/placeholder/300/200"
    ...
    setTimeout(() => {
      ...
      if (selectedCategories.length > 0) {
        ...
        filteredTeams = mockTeams.filter(team => 
          team.categories.some(cat => selectedCategoryNames.includes(cat))
        );
      }
  ...
  const TeamCard = ({ team }) => {
    ...
    const hiddenTagsCount = (team.categories || []).length - 2;
    const hasImage = Boolean(team.previewImage);
    const imageUrl = hasImage ? team.previewImage : null;
